...
// @ts-ignore
import {GroupDocument, GroupsDocument, GroupsQuery,} from './gql/groups.graphql';
// @ts-ignore
import {PopularProductsDocument, ProductsDocument,} from './gql/products.graphql';
// @ts-ignore
...
> = async ({ locale, params }) => {
  ...
  if (!types?.some((t) => t.slug === pageType)) {
    return {
      notFound: true,
      // This is required to regenerate the page
      revalidate: 120,
    };
  }
